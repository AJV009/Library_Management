...
<?php
	...
	$query="select * from msgarray where tous='".$_SESSION['UNAME']."'";
	...
	echo"
		<div class='w3-container w3-card-4 w3-green' style='margin-top:13%; margin-bottom:5%; margin-left:5%; margin-right:5%'>
			...
			<p>To message all students (Bulk Message) please type in 'ALLUC' in To User text feild.</p>
			<form method='POST'>
	...
	if(isset($_POST['send'])) {
		...
		if(mysqli_num_rows($request)>0){
			...
			echo"<script>alert('Your message has been send, but the receiver has to refresh his page to see the message.'); window.location.href='/LMS3/admi/msgs.php'</script>";
		}
		else{
			echo"<script>alert('No such user found. Message not delivered !!!'); window.location.href='/LMS3/admi/msgs.php'</script>";	
		}
	}
	if(isset($_POST['delete'])){
		$query="delete from msgarray where id=".$_POST['id']."";
		mysqli_query($conn,$query);
		echo"<script>alert('Message Deleted !!!'); window.location.href='/LMS3/admi/msgs.php';</script>";
	}
?>